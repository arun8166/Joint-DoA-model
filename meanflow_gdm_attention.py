...
class CFG:
    DATA_PATH = r"C:\Users\ugp.DESKTOP-7Q13T9G\Downloads\meanflow_gdm_attention\meanflow_gdm_attention\receive_echo_dataset_legacy.mat"
...
def find_data_path():
    ...
    print(f"[data] using {cands[0]} ({os.path.getsize(cands[0]) / 1e6:.0f} MB)")
    return cands[0]
...
def load_matfile(path):
    ...
    if az.ndim == 1:
        ...
        az = az.T
    assert y.shape[0] == 16 and y.ndim == 4, f"unexpected y_receive shape {y.shape}"
    assert az.shape[1] == CFG.N_ANGLES, f"unexpected target_azimuth shape {az.shape}"
    if np.abs(az).max() > np.pi + 1e-3:
        print("[data] WARNING: |azimuth| > pi -- values look like degrees, converting to radians")
        az = np.radians(az)
    if az.min() >= -1e-6 and az.max() > np.pi / 2 + 1e-3:
        print("[data] azimuths in [0, pi] (endfire) -> shifting to +/- pi/2 broadside")
        az = az - np.pi / 2
    ...
    print(f"[data] {uniq} unique azimuth values / {az.size} labels "
...
class DoaDataset(Dataset):
    def __init__(self, y, yc, az_rad, setup_idx, train, stats, label_grid_rad=0.0):
        self.y = y[..., setup_idx]
        self.yc = yc[..., setup_idx]
        self.az = az_rad[setup_idx]
        self.S = len(setup_idx)
        self.N_SNR = self.y.shape[2]
        self.N_SNAP = self.y.shape[1]
        self.train = train
        self.grid = float(label_grid_rad)
        self.p_mu, self.p_std = stats["p_mu"], stats["p_std"]

    def __len__(self):
        return self.S * self.N_SNR * self.N_SNAP

    def __getitem__(self, i):
        s, k, n = np.unravel_index(
            i,
            (self.S, self.N_SNR, self.N_SNAP)
        )
        ...
        if self.train:
            ph = np.exp(
                1j * np.float32(2 * np.pi * np.random.rand())
            ).astype(np.complex64)

            x, xc = x * ph, xc * ph

            if self.grid > 0:
                th = th + np.random.uniform(
                    -self.grid / 2,
                    self.grid / 2,
                    th.shape
                ).astype(np.float32)
...
def compute_stats(y, yc, setup_idx):
    ...
    stats = dict(p_mu=float(logp.mean()), p_std=float(logp.std() + 1e-6),
                 mf_scale=float(comp.std() + 1e-8))
    print(f"[stats] log-power mu/std = {stats['p_mu']:.3f}/{stats['p_std']:.3f}, "
...
class CDropout(nn.Module):
    ...
    def forward(self, z):
        if not self.training or self.p <= 0:
            return z
        keep = (torch.rand(z.shape, device=z.device) > self.p).float() / (1 - self.p)
        return z * keep
...
class CAttention(nn.Module):
    ...
    def forward(self, z):
        ...
        att = torch.einsum("bhid,bhjd->bhij", q, k.conj()).real / math.sqrt(self.dh)
        w = att.softmax(-1)
        outr = torch.einsum("bhij,bhjd->bhid", w, v.real)
        outi = torch.einsum("bhij,bhjd->bhid", w, v.imag)
        out = torch.complex(outr, outi).transpose(1, 2).reshape(B, T, self.h * self.dh)
        return self.o(out)
...
class CBlock(nn.Module):
    def __init__(self, d, heads, drop=0.1):
        ...
        self.ffn = nn.ModuleList([CLinear(d, 4 * d), ModReLU(4 * d), CLinear(4 * d, d)])
...
class ComplexEncoder(nn.Module):
    def __init__(self, n_ant=16, d=128, layers=5, heads=4, ctx=256, drop=0.1):
        ...
        self.blocks = nn.ModuleList([CBlock(d, heads, drop) for _ in range(layers)])
        self.lnf = CLayerNorm(d)
        self.pool_q = nn.Parameter(torch.randn(d, 2) * 0.02)
        self.mlp = nn.Sequential(nn.Linear(2 * d + 3, ctx), nn.GELU(), nn.Linear(ctx, ctx))
    ...
    def forward(self, z, extra, role):
        ...
        w = (torch.einsum("btd,d->bt", h, q.conj()).real / math.sqrt(self.d)).softmax(-1)
        gr = torch.einsum("bt,btd->bd", w, h.real)
        gi = torch.einsum("bt,btd->bd", w, h.imag)
        feat = torch.cat([gr, gi, torch.log(n2 + 1e-8),
                          extra.unsqueeze(-1), role.unsqueeze(-1)], dim=-1)
        return self.mlp(feat)

class TimeEmb(nn.Module):
    def __init__(self, dim=64):
        super().__init__()
        self.register_buffer("freqs", torch.exp(torch.linspace(0.0, math.log(200.0), dim // 2)))
...
class AdaBlock(nn.Module):
    def __init__(self, h):
        ...
        self.net = nn.Sequential(nn.Linear(h, 2 * h), nn.SiLU(), nn.Linear(2 * h, h))
...
class MeanFlowNet(nn.Module):
    def __init__(self, xdim=32, h=256, blocks=6, cdim=256):
        ...
        self.cmap = nn.Sequential(nn.Linear(cdim + 128, h), nn.SiLU(), nn.Linear(h, h))
...
class DiTBlock(nn.Module):
    def __init__(self, h, heads, cdim):
        ...
        self.mlp = nn.Sequential(nn.Linear(h, 4 * h), nn.SiLU(), nn.Linear(4 * h, h))
...
class AngleDenoiser(nn.Module):
    def __init__(self, h=128, blocks=4, heads=4, cond_in=512, cdim=256):
        ...
        self.cmap = nn.Sequential(nn.Linear(cond_in + 64, cdim), nn.SiLU(), nn.Linear(cdim, cdim))
        self.blocks = nn.ModuleList([DiTBlock(h, heads, cdim) for _ in range(blocks)])
        self.out_ln = nn.LayerNorm(h, elementwise_affine=False)
...
def angle_diffusion_loss(model, cond, u0):
    ...
    return F.mse_loss(model.ang(al * u0 + si * eps, tau, cond), al * eps - si * u0)
...
def ddim_angles(model, cond, steps):
    ...
    return z
@torch.no_grad()
def posterior_samples(model, x, pl, K, M, steps):
    B = x.size(0)
    cx = model.ctx_noisy(x, pl)
    if CFG.BASELINE:
...
@torch.no_grad()
def predict_single(model, stats, x_np, K=None, M=None, steps=None, device=None):
    model.eval()
    ...
    u = posterior_samples(model, x, pl, K, M, steps).reshape(-1,CFG.N_ANGLES).clamp(-0.999,0.999)
    th = torch.asin(u).sort(-1).values.cpu().numpy()
    return np.median(th, axis=0), th


@torch.no_grad()
def evaluate(model, dset, device, snapshots=(0,),
             K=4, M=8, steps=20, chunk=192):
    model.eval()
    db_scores = {
        k: [] for k in range(dset.N_SNR)
    }
    items = [
        (s, k, n)
    ...
    for c0 in tqdm(
        ...
        for (s, k, n) in sub:
            ...
            ks.append(k)
            trus.append(
                np.arcsin(np.sort(b["u"].numpy()))
            )
        x = torch.stack(xs).to(device)
        pl = torch.stack(pls).to(device)
        u = posterior_samples(
            model, x, pl, K, M, steps
        ).clamp(-0.999, 0.999)
        th_pred_rad = (
            torch.asin(u)
            .sort(-1).values
        ...
        for j, k in enumerate(ks):
            err_deg = np.degrees(
                th_pred_rad[j] - trus[j]
            )
            rmse_one_snapshot_deg = np.sqrt(
                np.mean(err_deg ** 2)
            )
            score_db = 10.0 * np.log10(
                max(rmse_one_snapshot_deg, 1e-9)
            )
...
def plot_rmse_db(metric_db, out_path, label):
    ...
    plt.plot(SNR_DB, [metric_db[k] for k in range(len(SNR_DB))], "o-", label=label)
    other = os.path.join(
        CFG.OUT_DIR,
    ...
    if os.path.exists(other):
        try:
            ...
            olab = (
                "joint (MeanFlow + diffusion)"
                if CFG.BASELINE else
                "baseline (no MeanFlow)"
            )
            plt.plot(
                SNR_DB,
                [o[f"{s}dB"] for s in SNR_DB],
                "s--",
                label=olab
            )
...
def main():
    ...
    print(f"[split] setups: train {len(idx_tr)} / val {len(idx_va)} / test {len(idx_te)} "
    ...
    dl = DataLoader(ds_tr, batch_size=CFG.BATCH, shuffle=True, num_workers=CFG.WORKERS,
    ...
    sched = torch.optim.lr_scheduler.LambdaLR(
        opt, lambda s: min(1.0, s / max(1, CFG.WARMUP)) *
    ...
    def save(ep):
        torch.save(dict(ema=ema.shadow, model=model.state_dict(),
                        opt=opt.state_dict(), sched=sched.state_dict(),
                        step=step, epoch=ep, stats=stats,
    ...
    for ep in range(ep0, CFG.EPOCHS):
        ...
        mf_m = an_m = 0.0
        nb = 0
        for batch in tqdm(dl, desc=f"epoch {ep + 1}/{CFG.EPOCHS}", leave=False):
            ...
            if (time.time() - t_start) > CFG.TIME_LIMIT_HOURS * 3600 - 900:
                out_of_time = True
                break
        print(f"[ep {ep + 1:03d}] mf={mf_m / max(nb, 1):.4f} ang={an_m / max(nb, 1):.4f} "
              f"p_tf={p_tf:.2f} lr={sched.get_last_lr()[0]:.2e}")

        if (ep + 1) % CFG.EVAL_EVERY == 0 or ep + 1 == CFG.EPOCHS or out_of_time:
            em = ema.build_eval_model(model)
            rmse = evaluate(em, ds_va, device, K=CFG.VAL_K, M=CFG.VAL_M, steps=CFG.VAL_STEPS)
            print("        val avg 10log10(RMSE_deg): " +
            "  ".join(f"{SNR_DB[k]}dB:{rmse[k]:.4f}" for k in range(len(SNR_DB))))
            save(ep + 1)
            del em
        if out_of_time:
            print("[time] approaching session limit -> checkpoint saved, stopping. "
    ...
    metric_te = evaluate(
    em, ds_te, device, snapshots=snaps,
    K=CFG.TEST_K, M=CFG.TEST_M, steps=CFG.TEST_STEPS
    )
    print("[TEST] average 10*log10(single-snapshot RMSE in degrees): " +
          "  ".join(f"{SNR_DB[k]}dB:{metric_te[k]:.4f}" for k in range(len(SNR_DB))))
    tag = "test_metrics_baseline.json" if CFG.BASELINE else "test_metrics.json"
    with open(os.path.join(CFG.OUT_DIR, tag), "w") as f:
        json.dump(
            dict(
                unit="dB",
                definition=f"mean_over_single_snapshots[10*log10(RMSE_deg_over_{CFG.N_ANGLES}_DoAs)]",
                avg_10log10_rmse_deg={
                    f"{SNR_DB[k]}dB": metric_te[k] for k in range(len(SNR_DB))
                },
            ),
            f,
            indent=2,
    ...
    s, k, n = 0, 3, 0                                  
    b = ds_te[int(np.ravel_multi_index((s, k, n), (ds_te.S, ds_te.N_SNR, ds_te.N_SNAP)))]
    x_raw = (b["x"].numpy() * 1.0)                     
    est, _ = predict_single(em, stats, x_raw, device=device)
    tru = np.arcsin(np.sort(b["u"].numpy()))
    print(f"[demo] truth (rad): {np.round(tru, 3)}  |  estimate (rad): {np.round(est, 3)}")
