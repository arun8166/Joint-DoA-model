...
def training_step(model, batch, p_tf):
    ...
    if CFG.GEN_CLEAN_ONLY:
        c_noisy_null = torch.zeros_like(cx)
        cond_ang = torch.cat([c_noisy_null, c_cl], dim=-1)
    else:
        cond_ang = torch.cat([cx, c_cl], dim=-1)
    ang_l = angle_diffusion_loss(model, cond_ang, u0)
...
def posterior_samples(model, x, pl, K, M, steps):
    ...
    if CFG.BASELINE:
        ...
        if CFG.GEN_CLEAN_ONLY:
            cx_doa = torch.zeros_like(cxr)
        else:
            cx_doa = cxr
        cond = torch.cat([cx_doa, c_cl], dim=-1)
...
def evaluate(model,
    ...
    for c0 in tqdm(range(0, len(items), chunk), desc="eval", leave=False):
        ...
        u = posterior_samples(model, x, pl, K, M, steps).clamp(-0.999, 0.999)
